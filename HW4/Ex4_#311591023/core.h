...
SC_MODULE( Core ) {
    ...
    float sc_lv_to_float(const sc_lv<32>& lv) {
        uint32_t int_rep = lv.to_uint();
        float f = *reinterpret_cast<float*>(&int_rep);
        return f;
    }
    sc_bv<34> float_to_bv(float value) {
        union {
            float f;
            uint32_t u;
        } converter;
        converter.f = value;
        sc_bv<34> bv;
        bv.range(33, 2) = sc_bv<32>(converter.u); // Store 32 bits of float in bits 2 to 33
        bv[1] = 0; // Just a placeholder bit
        bv[0] = 0; // Just a placeholder bit
        return bv;
    }
    sc_bv<34> int_to_bv(int value) {
        sc_bv<34> bv;
        bv.range(33, 2) = sc_bv<32>(value); // Store 32 bits of int in bits 2 to 33
        bv[1] = 0; // Just a placeholder bit
        bv[0] = 0; // Just a placeholder bit
        return bv;
    }
...
// //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            if (receive_data.is_01()) {
                ...
                if (first_two_bits == 2 && destination==5) {
                    store_begin_5 = 1;
                    // output5_f_from_r1.push_back(recovered_float);
                    
                } else if (first_two_bits == 1 && store_begin_5 == 1) {
                    store_begin_5 = 0;
                    output5_f_from_r1.push_back(recovered_float);
                    cout << "output5_f_from_r1.size() " <<output5_f_from_r1.size()<<endl;
                    maxp_start = true;
                } else if (store_begin_5==1){
                    output5_f_from_r1.push_back(recovered_float); 
                    // cout << "receive_data "<<receive_data<<endl;
                }
            }
            ...
            if (maxp_start) {
                int index = 0;
                std::vector<std::vector<std::vector<float>>> output5_from_r1(256, std::vector<std::vector<float>>(13, std::vector<float>(13)));
                for (int d = 0; d < 256; ++d) {
                    for (int h = 0; h < 13; ++h) {
                        for (int w = 0; w < 13; ++w) {
                            output5_from_r1[d][h][w] = output5_f_from_r1[index++];
                        }
                    }
                }
                auto pooledOutput5 = applyMaxPooling(output5_from_r1, 3, 2);
                pooledOutput5_f = flattenVector(pooledOutput5);  
                
                core5_send = true;
                maxp_start = false;
                cout << "core5 complete at time " << sc_time_stamp() << endl;
            }
            if (core5_send) {
                sc_bv<34> data_read_v;
                if (count_c5==0) {
                    data_read_v.range(33,32) = 10;
                    data_read_v.range(31,9) = "00000000000000000000000";
                    data_read_v.range(8,5) = "0100"; // destination  
                    data_read_v.range(4,0) = "00000";
                } else if (count_c5 == pooledOutput5_f.size()) {
                    data_read_v.range(33,32) = 01;
                    data_read_v.range(31,0) = float_to_sc_uint(pooledOutput5_f[count_c5-1]);
                    core5_send = false;
                } else {
                    data_read_v.range(33,32) = 00;
                    data_read_v.range(31,0) = float_to_sc_uint(pooledOutput5_f[count_c5-1]);       
            ...
            if (receive_data.is_01()) {
                ...
                if (first_two_bits == 2 && destination==4) {
                    store_begin_4 = 1;
                    
                } else if (first_two_bits == 1 && store_begin_4 == 1) {
                    store_begin_4 = 0;
                    output5_f_from_r5.push_back(recovered_float);
                    cout << "output5_f_from_r5.size() " <<output5_f_from_r5.size()<<endl;
                    adap_start = true;
                } else if (store_begin_4==1){
                    output5_f_from_r5.push_back(recovered_float); 
                }
            }
            ...
            if (adap_start) {
                int index = 0;
                std::vector<std::vector<std::vector<float>>> output5_from_r5(256, std::vector<std::vector<float>>(6, std::vector<float>(6)));
                for (int d = 0; d < 256; ++d) {
                    for (int h = 0; h < 6; ++h) {
                        for (int w = 0; w < 6; ++w) {
                            output5_from_r5[d][h][w] = output5_f_from_r5[index++];
                        }
                    }
                }
                auto finalOutput = adaptiveAvgPool(output5_from_r5, 6, 6);
                finalOutput_f = flattenVector(finalOutput);  
                
                core4_send = true;
                adap_start = false;
                cout << "core4 complete at time " << sc_time_stamp() << endl;
            }
            if (core4_send) {
                sc_bv<34> data_read_v;
                if (count_c4==0) {
                    data_read_v.range(33,32) = 10;
                    data_read_v.range(31,9) = "00000000000000000000000";
                    data_read_v.range(8,5) = "0110"; // destination  
                    data_read_v.range(4,0) = "00000";
                } else if (count_c4 == finalOutput_f.size()) {
                    data_read_v.range(33,32) = 01;
                    data_read_v.range(31,0) = float_to_sc_uint(finalOutput_f[count_c4-1]);
                    core4_send = false;
                } else {
                    data_read_v.range(33,32) = 00;
                    data_read_v.range(31,0) = float_to_sc_uint(finalOutput_f[count_c4-1]);       
